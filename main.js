$(document).ready(function() {
  $("#newsSection").change(function() {
    ...
    //Empty articles conatiner on news section switch
    ...
    $.ajax({
      url:
        ...
        section +
        ".json?api-key=JZGkmnwVvjwt6MH5Fh2i4dL1nQTF2AD9",
      type: "GET",
      datatype: "json"
    })
      ...
      .fail(function() {
        ...
        $(".loadingcontainer").append(
          "<h2 class='error'>We're sorry but the New York Times API could not be reached at this time</h2>"
        );
      })
      ...
      .done(function(topStories) {
        ...
        $.each(topStories.results, function() {
          //Ends when 12 articles have been rendered
          if (
            document.querySelector(".articlesContainer").childElementCount ===
            12
          ) {
            return false;
          } else {
            ...
            if (
              ...
              $(".newArticle").append(
                "<div><p>" + this.abstract + "</p></div>"
              );
              $(".newArticle").css(
                "background-image",
                "url(" + this.multimedia[4].url + ")"
              );
